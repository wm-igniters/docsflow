...
export async function POST(req: Request) {
  try {
    ...
    // 1. Get all modified documents
    const modifiedDocs = await TechStackModel.find({ status: { $ne: 'published' } });
    ...
    const { data: pr } = await octokit.rest.pulls.create({
      ...
      head: newBranchName,
      base: BRANCH,
      title: `docs(tech-stack): update definitions via DocsFlow [${new Date().toLocaleDateString()}]`,
